...
const DataEntriesSearch = () => {
  ...
  return (
    <div style={{ marginTop: "9rem" }}>
      <h3 style={{ color: "black" }}>Search Data Entries</h3>
      ...
      {loadingUser && <p style={{ color: "black" }}>Loading user data...</p>}
      {userData &&
        userData.dataEntriesByUser.map((entry) => (
          <div style={{marginTop: "0.5rem"}} key={entry.id}>
            <p style={{ color: "black" }}>User: {entry.user}</p>
            <p style={{ color: "black" }}>Model: {entry.model}</p>
            <p style={{ color: "black" }}>Prompt: {entry.prompt}</p>
            <p style={{ color: "black" }}>Result: {entry.result}</p>
          </div >
        ))}

      {loadingModel && <p style={{ color: "black" }}>Loading model data...</p>}
      {modelData &&
